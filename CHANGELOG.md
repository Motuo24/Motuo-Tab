...
- 主题模式：新增 浅色 / 深色 / 自动 切换；自动模式根据壁纸像素实时重着色文字
- 帮助中心：菜单新增「帮助」入口，Markdown 式布局（左侧章节导航 + 右侧内容），覆盖 AI 密钥设置、博查搜索密钥教程、快捷方式操作说明

### 变更

- 快捷方式弹窗：重构视觉（按键分离键帽、按键由少到多排序、描述文案更柔和），并补充 Command 键等价快捷键
- 菜单重构：菜单项图标化、分组展示、危险操作隔离、站内搜索直达改为胶囊卡片
- 交互优化：点击右下角菜单按钮可关闭从菜单打开的弹窗；修复菜单按钮被弹窗遮罩遮挡的问题
- 默认快捷方式：刷新内置默认项（含图片图标）
- AI 助手：侧边栏标题替换为品牌图标
