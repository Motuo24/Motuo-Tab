...
>
> ⚠️ **例外：AI 助手（含联网搜索）仅在安装为浏览器扩展时可用。** 用 `file://` 直接打开本地单文件版时，页面没有扩展的 host 权限，浏览器会拦截跨域请求（表现为请求挂起 / 超时 / 失败），无法连接模型与搜索接口。请使用**扩展版**，或通过具备正常来源的在线版 / 本地 http 服务器访问。
...
> **注意**：AI 助手需要跨域请求模型接口与博查搜索 API，**仅在浏览器扩展版中可用**（扩展已声明 `<all_urls>` host 权限）。用 `file://` 打开 `dist/newtab.html` 时页面没有该权限，请求会被浏览器拦截，AI 功能无法使用；如必须使用单文件版，请通过本地 http 服务器或在线托管（`http/https`）访问。
