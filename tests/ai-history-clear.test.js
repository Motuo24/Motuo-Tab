...
// 搜索链路 stub：chat#1 → <web_search>文本协议；web-search → 结果；chat#2 → 基于结果的最终回答
function searchChainStub() {
  ...
  return (url) => {
    if (String(url).includes('/chat/completions')) {
      chatSeq++;
      if (chatSeq === 1) {
        return sseResponse([{ choices: [{ delta: { content: '<web_search>最新AI工具</web_search>' } }] }]);
      }
      return sseResponse([
...
const LEAK_SNIPPET = '这里是最新AI工具的汇总页面'; // 搜索结果正文（不应渲染给用户）
...
test('问题三：重进页面后不再外露上一次的联网搜索结果（结果仅作 API 上下文保留）', async () => {
  // ===== 第一次进入：跑完一轮联网搜索 =====
  ...
  await waitFor(() => s1.document.querySelectorAll('.ai-msg-assistant').length >= 1);
  ...
  // localStorage 历史中确实持久化了搜索结果（含原文），且标记为 hidden
  const history1 = JSON.parse(s1.window.localStorage.getItem('newtab.ai.history.v1'));
  const resultsMsg = history1.find((m) => m.content && String(m.content).includes(LEAK_SNIPPET));
  assert.ok(resultsMsg, '历史中应保留搜索结果（API 上下文）');
  assert.strictEqual(resultsMsg.hidden, true, '搜索结果消息应标记 hidden，界面不渲染');
  ...
  assert.ok(!text1.includes(LEAK_SNIPPET), '重开面板后不应渲染搜索结果');
  ...
  // API 上下文完整性不受影响：搜索结果仍保留在持久化历史中
  const history2 = JSON.parse(s2.window.localStorage.getItem('newtab.ai.history.v1'));
  assert.ok(history2.find((m) => m.content && String(m.content).includes(LEAK_SNIPPET)), '搜索结果应保留在 localStorage');
});
